...
class O3_CPU {
	...
	uint64_t instr_unique_id = 0, completed_executions = 0,
		 begin_sim_cycle = 0, begin_sim_instr, last_sim_cycle = 0,
		 last_sim_instr = 0, finish_sim_cycle = 0, finish_sim_instr = 0,
	...
	// RTP0 for pcache
	std::queue<std::vector<LSQ_ENTRY>::iterator> RTP0;

	// Ready-To-Store
	std::queue<std::vector<LSQ_ENTRY>::iterator> RTS0, RTS1;

	// branch
	int branch_mispredict_stall_fetch =
		0; // flag that says that we should stall because a branch prediction was wrong
	...
	// TLBs and caches
	CACHE ITLB{ "ITLB",	    ITLB_SET,	      ITLB_WAY,
		    ITLB_WQ_SIZE,   ITLB_RQ_SIZE,     ITLB_PQ_SIZE,
		    ITLB_MSHR_SIZE, ITLB_HIT_LATENCY, ITLB_FILL_LATENCY,
		    ITLB_MAX_READ,  ITLB_MAX_WRITE,   ITLB_PREF_LOAD,
		    false },
		DTLB{ "DTLB",	      DTLB_SET,		DTLB_WAY,
		      DTLB_WQ_SIZE,   DTLB_RQ_SIZE,	DTLB_PQ_SIZE,
		      DTLB_MSHR_SIZE, DTLB_HIT_LATENCY, DTLB_FILL_LATENCY,
		      DTLB_MAX_READ,  DTLB_MAX_WRITE,	DTLB_PREF_LOAD,
		      false },
		STLB{ "STLB",	      STLB_SET,		STLB_WAY,
		      STLB_WQ_SIZE,   STLB_RQ_SIZE,	STLB_PQ_SIZE,
		      STLB_MSHR_SIZE, STLB_HIT_LATENCY, STLB_FILL_LATENCY,
		      STLB_MAX_READ,  STLB_MAX_WRITE,	STLB_PREF_LOAD,
		      false },
		L1I{ "L1I",	    L1I_SET,	     L1I_WAY,
		     L1I_WQ_SIZE,   L1I_RQ_SIZE,     L1I_PQ_SIZE,
		     L1I_MSHR_SIZE, L1I_HIT_LATENCY, L1I_FILL_LATENCY,
		     L1I_MAX_READ,  L1I_MAX_WRITE,   L1I_PREF_LOAD,
		     true },
		L1D{ "L1D",	    L1D_SET,	     L1D_WAY,
		     L1D_WQ_SIZE,   L1D_RQ_SIZE,     L1D_PQ_SIZE,
		     L1D_MSHR_SIZE, L1D_HIT_LATENCY, L1D_FILL_LATENCY,
		     L1D_MAX_READ,  L1D_MAX_WRITE,   L1D_PREF_LOAD,
		     L1D_VA_PREF },
		L1P{ "L1P",	    L1P_SET,	     L1P_WAY,
		     L1P_WQ_SIZE,   L1P_RQ_SIZE,     L1P_PQ_SIZE,
		     L1P_MSHR_SIZE, L1P_HIT_LATENCY, L1P_FILL_LATENCY,
		     L1P_MAX_READ,  L1P_MAX_WRITE,   L1P_PREF_LOAD,
		     L1P_VA_PREF },
		L2C{ "L2C",	    L2C_SET,	     L2C_WAY,
		     L2C_WQ_SIZE,   L2C_RQ_SIZE,     L2C_PQ_SIZE,
		     L2C_MSHR_SIZE, L2C_HIT_LATENCY, L2C_FILL_LATENCY,
	...
	PageTableWalker PTW{ "PTW",	   cpu,		  PSCL5_SET,
			     PSCL5_WAY,	   PSCL4_SET,	  PSCL4_WAY,
			     PSCL3_SET,	   PSCL3_WAY,	  PSCL2_SET,
			     PSCL2_WAY,	   PTW_RQ_SIZE,	  PTW_MSHR_SIZE,
			     PTW_MAX_READ, PTW_MAX_WRITE, 0 };

	// constructor
	O3_CPU(uint32_t cpu, uint64_t warmup_instructions,
	       uint64_t simulation_instructions)
		: cpu(cpu), begin_sim_cycle(warmup_instructions),
		  ...
		  simulation_instructions(simulation_instructions)
	{
		...
		PTW.lower_level =
			&L1D; //PTW checks L1 cache for cached translation blocks.
		...
		ITLB.find_victim = std::bind(&CACHE::lru_victim, &ITLB, _1, _2,
					     _3, _4, _5, _6, _7);
		DTLB.find_victim = std::bind(&CACHE::lru_victim, &DTLB, _1, _2,
					     _3, _4, _5, _6, _7);
		STLB.find_victim = std::bind(&CACHE::lru_victim, &STLB, _1, _2,
					     _3, _4, _5, _6, _7);
		L1I.find_victim = std::bind(&CACHE::lru_victim, &L1I, _1, _2,
					    _3, _4, _5, _6, _7);
		L1D.find_victim = std::bind(&CACHE::lru_victim, &L1D, _1, _2,
					    _3, _4, _5, _6, _7);
		L1P.find_victim = std::bind(&CACHE::lru_victim, &L1P, _1, _2,
					    _3, _4, _5, _6, _7);
		L2C.find_victim = std::bind(&CACHE::lru_victim, &L2C, _1, _2,
					    _3, _4, _5, _6, _7);

		ITLB.update_replacement_state =
			std::bind(&CACHE::lru_update, &ITLB, _2, _3, _7, _8);
		DTLB.update_replacement_state =
			std::bind(&CACHE::lru_update, &DTLB, _2, _3, _7, _8);
		STLB.update_replacement_state =
			std::bind(&CACHE::lru_update, &STLB, _2, _3, _7, _8);
		L1I.update_replacement_state =
			std::bind(&CACHE::lru_update, &L1I, _2, _3, _7, _8);
		L1D.update_replacement_state =
			std::bind(&CACHE::lru_update, &L1D, _2, _3, _7, _8);
		L1P.update_replacement_state =
			std::bind(&CACHE::lru_update, &L1P, _2, _3, _7, _8);
		L2C.update_replacement_state =
			std::bind(&CACHE::lru_update, &L2C, _2, _3, _7, _8);

		ITLB.replacement_final_stats =
			std::bind(&CACHE::lru_final_stats, &ITLB);
		DTLB.replacement_final_stats =
			std::bind(&CACHE::lru_final_stats, &DTLB);
		STLB.replacement_final_stats =
			std::bind(&CACHE::lru_final_stats, &STLB);
		L1I.replacement_final_stats =
			std::bind(&CACHE::lru_final_stats, &L1I);
		L1D.replacement_final_stats =
			std::bind(&CACHE::lru_final_stats, &L1D);
		L1P.replacement_final_stats =
			std::bind(&CACHE::lru_final_stats, &L1P);
		L2C.replacement_final_stats =
			std::bind(&CACHE::lru_final_stats, &L2C);
	}

	// functions
	uint32_t init_instruction(ooo_model_instr instr);
	void check_dib(), translate_fetch(), fetch_instruction(),
		promote_to_decode(), decode_instruction(),
		dispatch_instruction(), schedule_instruction(),
		...
		do_scheduling(
			champsim::circular_buffer<ooo_model_instr>::iterator
				rob_it),
		do_execution(champsim::circular_buffer<ooo_model_instr>::iterator
				     rob_it),
	...
	void add_load_queue(
		...
		uint32_t data_index),
		add_store_queue(champsim::circular_buffer<
					ooo_model_instr>::iterator rob_index,
				uint32_t data_index),
		add_pcache_load_queue(champsim::circular_buffer<
					ooo_model_instr>::iterator rob_index,
				uint32_t data_index);
	...
	// branch predictor
	uint8_t predict_branch(uint64_t ip, uint64_t predicted_target,
			       uint8_t always_taken, uint8_t branch_type);
	void initialize_branch_predictor(),
		last_branch_result(uint64_t ip, uint64_t branch_target,
				   uint8_t taken, uint8_t branch_type);

	// btb
	std::pair<uint64_t, uint8_t> btb_prediction(uint64_t ip,
						    uint8_t branch_type);
	void initialize_btb(), update_btb(uint64_t ip, uint64_t branch_target,
					  uint8_t taken, uint8_t branch_type);
	...
	void l1i_prefetcher_cache_fill(uint64_t v_addr, uint32_t set,
