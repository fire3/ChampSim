...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
	...
	cout << cache->NAME;
	cout << " TRANSLATION ACCESS: " << setw(10) << cache->roi_access[cpu][4]
...
void print_branch_stats()
{
	for (uint32_t i = 0; i < NUM_CPUS; i++) {
		...
		cout << " Average ROB Occupancy at Mispredict: "
		     << (1.0 *
			 ooo_cpu[i].total_rob_occupancy_at_branch_mispredict) /
				ooo_cpu[i].branch_mispredictions
		     << endl;
	...
	*/
		...
		cout << "BRANCH_DIRECT_JUMP: "
		     << (1000.0 * ooo_cpu[i].branch_type_misses[1] /
			 (ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr))
		     << endl;
		cout << "BRANCH_INDIRECT: "
		     << (1000.0 * ooo_cpu[i].branch_type_misses[2] /
			 (ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr))
		     << endl;
		cout << "BRANCH_CONDITIONAL: "
		     << (1000.0 * ooo_cpu[i].branch_type_misses[3] /
			 (ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr))
		     << endl;
		cout << "BRANCH_DIRECT_CALL: "
		     << (1000.0 * ooo_cpu[i].branch_type_misses[4] /
			 (ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr))
		     << endl;
		cout << "BRANCH_INDIRECT_CALL: "
		     << (1000.0 * ooo_cpu[i].branch_type_misses[5] /
			 (ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr))
		     << endl;
		cout << "BRANCH_RETURN: "
		     << (1000.0 * ooo_cpu[i].branch_type_misses[6] /
			 (ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr))
		     << endl
		     << endl;
	}
...
void print_dram_stats()
{
	...
	if (DRAM.dbus_congested[NUM_TYPES][NUM_TYPES])
		cout << " AVG_CONGESTED_CYCLE: "
			 ...
			 DRAM.dbus_congested[NUM_TYPES][NUM_TYPES])
		     << endl;
	else
...
void finish_warmup()
{
	...
	for (uint32_t i = 0; i < NUM_CPUS; i++) {
		...
		cout << " (Simulation time: " << elapsed_hour << " hr "
		     << elapsed_minute << " min " << elapsed_second << " sec) "
		     << endl;
...
{
	cout << "DEADLOCK! CPU " << i
	     << " instr_id: " << ooo_cpu[i].ROB.front().instr_id;
	...
	for (uint32_t j = 0; j < LQ_SIZE; j++) {
		cout << "[LQ] entry: " << j
		     << " instr_id: " << ooo_cpu[i].LQ[j].instr_id
		     << " address: " << hex << ooo_cpu[i].LQ[j].physical_address
		     << dec << " translated: " << +ooo_cpu[i].LQ[j].translated
		     << " fetched: " << +ooo_cpu[i].LQ[i].fetched << endl;
	}
	...
	for (uint32_t j = 0; j < SQ_SIZE; j++) {
		cout << "[SQ] entry: " << j
		     << " instr_id: " << ooo_cpu[i].SQ[j].instr_id
		     << " address: " << hex << ooo_cpu[i].SQ[j].physical_address
		     << dec << " translated: " << +ooo_cpu[i].SQ[j].translated
		     << " fetched: " << +ooo_cpu[i].SQ[i].fetched << endl;
	}
	...
	for (PACKET &entry : ooo_cpu[i].L1D.MSHR) {
		std::cout << "[L1D MSHR] entry: " << j
			  << " instr_id: " << entry.instr_id;
		std::cout << " address: " << std::hex << entry.address
			  << " full_addr: " << entry.full_addr << std::dec
			  << " type: " << +entry.type;
		std::cout << " fill_level: " << entry.fill_level
			  << " event_cycle: " << entry.event_cycle << std::endl;
		++j;
	}
...
//	if (use_tsp || use_rmm || use_pcache || use_direct_segment || use_dcache_ptable) {
	...
	std::cout << "VirtualMemory page size: " << PAGE_SIZE
		  << " log2_page_size: " << LOG2_PAGE_SIZE << std::endl;
	...
	for (int i = 0; i < argc; i++) {
		if (found_traces) {
			std::cout << "CPU " << traces.size() << " runs "
				  << argv[i] << std::endl;

			traces.push_back(
				get_tracereader(argv[i], i, knob_cloudsuite));
	...
	for (int i = 0; i < NUM_CPUS; i++) {
		ooo_cpu.emplace_back(i, warmup_instructions,
				     simulation_instructions);
		ooo_cpu.at(i).L1I.l1i_prefetcher_cache_operate =
			cpu_l1i_prefetcher_cache_operate;
		ooo_cpu.at(i).L1I.l1i_prefetcher_cache_fill =
			cpu_l1i_prefetcher_cache_fill;
		...
		LLC.find_victim = std::bind(&CACHE::llc_find_victim, &LLC, _1,
					    _2, _3, _4, _5, _6, _7);
		LLC.update_replacement_state =
			std::bind(&CACHE::llc_update_replacement_state, &LLC,
				  _1, _2, _3, _4, _5, _6, _7, _8);
		LLC.replacement_final_stats =
			std::bind(&CACHE::lru_final_stats, &LLC);
	...
	while (run_simulation) {
		...
		for (int i = 0; i < NUM_CPUS; i++) {
			...
			// read from trace
			if (!ooo_cpu[i].IFETCH_BUFFER.full() &&
			    (ooo_cpu[i].fetch_stall == 0)) {
				while (ooo_cpu[i].init_instruction(
					traces[i]->get()))
					;
			}
			...
			if (show_heartbeat &&
				...
				if (warmup_complete[i])
					cumulative_ipc =
						(1.0 *
						 (ooo_cpu[i].num_retired -
						  ooo_cpu[i].begin_sim_instr)) /
						(current_core_cycle[i] -
						 ooo_cpu[i].begin_sim_cycle);
				else
					cumulative_ipc =
						(1.0 * ooo_cpu[i].num_retired) /
						current_core_cycle[i];
				float heartbeat_ipc =
					(1.0 * ooo_cpu[i].num_retired -
					 ooo_cpu[i].last_sim_instr) /
					(current_core_cycle[i] -
					 ooo_cpu[i].last_sim_cycle);
				...
				cout << " (Simulation time: " << elapsed_hour
				     << " hr " << elapsed_minute << " min "
				     << elapsed_second << " sec) " << endl;
				ooo_cpu[i].next_print_instruction +=
					STAT_PRINTING_PERIOD;
			...
			// check for deadlock
			if (!ooo_cpu[i].ROB.empty() &&
			    (ooo_cpu[i].ROB.front().event_cycle +
			...
			if (all_warmup_complete ==
			    NUM_CPUS) { // this part is called only once when all cores are warmed up
				all_warmup_complete++;
				finish_warmup();
			}
